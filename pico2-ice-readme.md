...
2. **Clock Start**: Due to a hardware bug with CDONE voltage levels, the RP2350B immediately starts the **25.175MHz clock** on `PIN_CLOCK` (GPIO21) using PIO
...
```
**Complex Triggering**
Note:  GPIO45 and GPIO46 (the two lowest pins just to the right of the USB C connector) must be jumpered if you want to use COMPLEX triggering (triggering on a pattern).  You can change these LogicAnalyzer_Board_Settings.h if you want other pins on the RP2350B.
**Important Notes:**
...
The FPGA receives a **25.175MHz clock** generated by the RP2350B's PIO system.

### Changing the FPGA Clock Frequency

To modify the clock frequency, change the `FPGA_CLOCK_FREQUENCY` define in `LogicAnalyzer_Board_Settings.h` for the `BUILD_PICO2_ICE` configuration.

For example, to set 10MHz:
```c
#define FPGA_CLOCK_FREQUENCY 10000000.0f
```
...
2. Verify the 25.175MHz clock is present on GPIO21 (Channel 02)
...
- Check GPIO21 with oscilloscope for 25.175MHz signal
...
- **FPGA Clock**: 25.175MHz (user configurable)
